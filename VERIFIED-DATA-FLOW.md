...
The wallet page was pricing holdings from the first page of the market list, and the list answers a bounded number of rows at a time, so anything past that page showed no price, no logo and no share however ordinary the holding was. Rows are now looked up by the addresses actually held, in batches. Checked against a live wallet: every holding priced, with its logo.
