...
function listShell(){
 ...
 '<div class="filters"><select id="version-filter" aria-label="Pool version"><option value="">All versions</option><option value="v2">V2</option><option value="v3">V3</option><option value="v4">V4</option></select><select id="venue-filter" aria-label="DEX"><option value="">All DEXes</option>'+Object.entries(VENUES).map(([id,label])=>'<option value="'+id+'">'+label+'</option>').join('')+'</select><label>MIN LIQ<input id="min-liquidity" type="number" min="0" placeholder="$0"></label><label>MIN VOL<input id="min-volume" type="number" min="0" placeholder="$0"></label><button id="clear-filters" class="muted">Reset</button></div>'+ 
...
function paintDetail(d){
 ...
 // each of transactions, money and people split into the buying and the selling side. A figure we have not
 // measured shows a dash; none of them is estimated from another.
 const pair=(label,a,b,aLabel,bLabel,ratio)=>'<div class="pair"><div class="pair-head"><small>'+label+'</small>'
  +'<small class="up">'+aLabel+'</small><small class="down">'+bLabel+'</small></div>'
...
async function loadSame(){
 ...
 if(!symbol){$('same').innerHTML='<tr><td colspan="5" class="empty">This token has no symbol to match.</td></tr>';return;}
 ...
 try{
  ...
  if(!rows.length){
   sameFor=null;
   $('same').innerHTML='<tr><td colspan="5" class="empty">No other token is trading under this ticker.</td></tr>';return;
  }
  $('same').innerHTML=rows.map(r=>'<tr class="same-row"><td><a class="token-cell" href="/token/'+esc(r.address)+'">'+icon(r)+'<span><strong>'+esc(r.symbol||'?')+(r.address===oldest?'<span class="og-tag" title="The oldest contract we have indexed under this ticker">OG</span>':'')+'</strong><small class="muted">'+esc(short(r.address))+'</small></span></a></td><td>'+price(r.price)+'</td><td>'+usd(r.marketCap)+'</td><td>'+usd(r.volume)+'</td><td>'+age(r.createdAt)+'</td></tr>').join('');
 }catch(e){
  if(e.name==='AbortError')return;
  sameFor=null;
  $('same').innerHTML='<tr><td colspan="5" class="empty">Ticker search unavailable right now.</td></tr>';
 }
}
...
// loaded from somewhere: one less request, and nothing to break when a host goes away.
function usdcMark(){
 return '<span class="token-icon usdc-mark"><svg viewBox="0 0 32 32" aria-hidden="true">'
  +'<circle cx="16" cy="16" r="16" fill="#2775ca"/>'
  +'<path fill="#fff" d="M16 5.8c-5.6 0-10.2 4.6-10.2 10.2S10.4 26.2 16 26.2 26.2 21.6 26.2 16 21.6 5.8 16 5.8zm0 18.5c-4.6 0-8.3-3.7-8.3-8.3S11.4 7.7 16 7.7s8.3 3.7 8.3 8.3-3.7 8.3-8.3 8.3z"/>'
  +'<path fill="#fff" d="M17 15.2c-1.9-.5-2.5-.8-2.5-1.6 0-.7.6-1.2 1.6-1.2.9 0 1.5.4 1.8 1.2h1.9c-.3-1.4-1.2-2.3-2.6-2.6v-1.4h-2v1.4c-1.6.3-2.7 1.4-2.7 2.7 0 1.7 1.3 2.3 3.1 2.8 1.7.4 2.2.8 2.2 1.6 0 .8-.7 1.3-1.8 1.3-1.2 0-1.9-.5-2.1-1.4h-1.9c.2 1.5 1.2 2.5 2.9 2.8v1.4h2v-1.4c1.7-.3 2.8-1.4 2.8-2.9 0-1.7-1.2-2.4-2.7-2.7z"/>'
  +'</svg></span>';
}
